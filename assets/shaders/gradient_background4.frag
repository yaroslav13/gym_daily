...
void main() {
  ...
  // Определяем цвета для градиента, основанные на вашей картинке
  vec3 lightBlue = vec3(0.4, 0.7, 0.9);     // Голубой
  vec3 green = vec3(0.2, 0.8, 0.5);         // Зеленый
  vec3 lightGrey = vec3(0.9, 0.9, 0.9);     // Светло-серый (почти белый)

  // Создаем плавный переход между цветами на основе шума
  vec3 color;
  if (finalNoise < 0.33) {
    // От светло-серого к голубому
    float t = finalNoise / 0.33;
    color = mix(lightGrey, lightBlue, t);
  } else if (finalNoise < 0.66) {
    // От голубого к зеленому
    float t = (finalNoise - 0.33) / 0.33;
    color = mix(lightBlue, green, t);
  } else {
    // От зеленого к светло-серому (для плавного перехода при повторении)
    float t = (finalNoise - 0.66) / 0.34;
    color = mix(green, lightGrey, t);
  }

  // Добавляем зернистость
  float grainAmount = 0.1; // Интенсивность зернистости
  ...
  float glow = 0.03 * sin(uTime * 0.1 + uv.x * 10.0) * sin(uTime * 0.13 + uv.y * 10.0);
  color += glow;
